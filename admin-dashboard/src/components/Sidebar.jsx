...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <aside
      className={`w-64 h-screen bg-white border-r border-gray-100 fixed left-0 top-0 flex flex-col z-50 transition-transform duration-300 lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
    >
      <div className="p-8 flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <Link to="/" className="block">
            <img src="/assets/logo.svg" alt="hubröbé" className="h-4 md:h-5" />
          </Link>
          <span className="text-[10px] uppercase tracking-widest text-black/40 font-bold font-sofia-pro">
            Admin Panel
          </span>
        </div>
